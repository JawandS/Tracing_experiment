...
do
  ...
  sudo bpftrace context_switch_probe.bt >> raw.txt & while inotifywait -e close_write tracking.txt; do ((counter=counter+1)) && if [ $counter -ge 20 ]; then kill $(ps aux | grep '[b]pftrace' | awk '{print $2}'); fi; done &
  for a_var in {1...20}
  do
