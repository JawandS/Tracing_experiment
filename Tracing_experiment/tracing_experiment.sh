...
do
  ...
  sudo bpftrace context_switch_probe.bt >> raw.txt & while inotifywait -e modify tracking.txt; do ((counter=counter+1)) && echo "$counter" && if [ "$counter" -ge 20 ]; then kill "$(ps aux | grep '[b]pftrace' | awk '{print $2}')" && echo "killing jobs"; fi; done &
  for a_var in {1...20}
  do
